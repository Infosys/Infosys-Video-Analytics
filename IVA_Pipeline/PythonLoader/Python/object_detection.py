...
class ObjectDetection(object):
    ...
    def predict_image(self, image, confidence_threshold):
        inputs = self.preprocess(image)
        prediction_outputs = self.predict(inputs)
        return self.postprocess(prediction_outputs, confidence_threshold)
    ...
    def postprocess(self, prediction_outputs, confidence_threshold):
        ...
        index, = np.where(max_probs > float(confidence_threshold))
        ...
        return [{'Cs': round(float(selected_probs[i]), 8),
                 #'tagId': int(selected_classes[i]),
                 'Lb': self.labels[selected_classes[i]],
                 'Dm': {
                     'X': round(float(selected_boxes[i][0]), 8),
                     'Y': round(float(selected_boxes[i][1]), 8),
                     'H': round(float(selected_boxes[i][2]), 8),
                     'W': round(float(selected_boxes[i][3]), 8)
                 }
